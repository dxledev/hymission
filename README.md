...
| `hide_namespaces_overview` | string | `hypr-dock,waybar,chromack,wardnc,wardbnc,dashboard,rofi` | Comma-separated layer-surface namespaces to hide with the same strip handoff in the regular, non-scrolling overview, including non-exclusive surfaces. Exclusive bars are still hidden regardless of namespace. |
| `hide_layers_when_overview` | bool | `1` | Hide matching non-window layer surfaces while multi-workspace Mission Control is shown. |
| `hide_namespace_overview_niri_scrolling` | string | `chromack,wardnc,wardbnc,dashboard,rofi` | Comma-separated layer-surface namespaces to capture and scale with the workspace in the scrolling overview. |
