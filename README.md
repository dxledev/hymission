...
| `niri_mode_wallpaper_zoom` | bool | `0` | Single-workspace scrolling niri mode only. Capture the full monitor background, reserved work-area space, and top-layer surfaces with positive exclusive zones when overview opens and render them without blur inside each animated workspace viewport, including empty workspaces. When disabled, workspace viewports use the blurred placeholder card. |
...
| `niri_mode_wallpaper_zoom_layer_namespaces` | string | `awww-daemon` | Comma-separated ECMAScript regular expressions matched against complete background and bottom layer namespaces for capture, hiding, and workspace zoom. Plain names remain exact matches; use `.*` for a wildcard. |
| `niri_mode_wallpaper_zoom_layer_refresh_ms` | int | `100` | Refresh interval for retained layout-reserving layer snapshots such as Waybar and docks while overview is open. Set to `0` to disable live refresh. Values below `16` are clamped to `16`. |
...
| `hide_namespaces_overview` | string | `hypr-dock,waybar,chromack,wardnc,wardbnc,dashboard,rofi` | Comma-separated ECMAScript regular expressions matched against complete layer-surface namespaces to hide with the same strip handoff in the regular, non-scrolling overview, including non-exclusive surfaces. Plain names remain exact matches; use `.*` for a wildcard. Exclusive bars are still hidden regardless of namespace. |
| `hide_layers_when_overview` | bool | `1` | Hide matching non-window layer surfaces while multi-workspace Mission Control is shown. |
| `hide_namespace_overview_niri_scrolling` | string | `chromack,wardnc,wardbnc,dashboard,rofi` | Comma-separated ECMAScript regular expressions matched against complete layer-surface namespaces to capture and scale with the workspace in the scrolling overview. Plain names remain exact matches; use `.*` for a wildcard. |
