...
NiriWallpaperViewportShadowConfig niriWallpaperViewportShadowConfig(HANDLE handle, const Rect& renderRect) {
    ...
    const double softness = std::clamp(getConfigFloat(handle, "plugin:hymission:niri_mode_wallpaper_zoom_shadow_softness", 140.0), 0.0, 1024.0);
    ...
    const double extentScale = std::clamp(getConfigFloat(handle, "plugin:hymission:niri_mode_wallpaper_zoom_shadow_extent_scale", 0.225), 0.0, 4.0);
...
std::optional<ScrollingOverviewGeometry> scrollingOverviewTapeRowGeometryForWindow(const PHLWINDOW& window, const Rect& fallbackGlobal, PHLWINDOW anchorWindow = {},
    ...
    const Rect        fallbackSourceGlobal = fallbackGlobal;
    ...
    for (const auto& candidate : targetColumn.column->targetDatas) {
        ...
        targetSource = centeredSurfaceRectInLayoutBox(virtualBox, fallbackSourceGlobal);
...
void OverviewController::captureNiriOverviewViewports() {
    m_niriOverviewViewportSnapshots.clear();
    if (!niriModeEnabled())
        return;
    ...
    for (const auto& monitor : g_pCompositor->m_monitors) {
        ...
        const Rect viewport = makeRect(monitorBox.x, monitorBox.y, monitorBox.width, monitorBox.height);
        Rect       initialWorkArea;
        if (monitor->m_activeWorkspace && monitor->m_activeWorkspace->m_space) {
            ...
            initialWorkArea = makeRect(liveWorkArea.x, liveWorkArea.y, liveWorkArea.width, liveWorkArea.height);
        }
        if (initialWorkArea.width <= 1.0 || initialWorkArea.height <= 1.0) {
            const CBox monitorWorkArea = monitor->logicalBoxMinusReserved();
            initialWorkArea = makeRect(monitorWorkArea.x, monitorWorkArea.y, monitorWorkArea.width, monitorWorkArea.height);
        }
        if (viewport.width <= 1.0 || viewport.height <= 1.0)
            continue;

        m_niriOverviewViewportSnapshots.push_back({
            .monitorId = monitor->m_id,
            .viewportGlobal = viewport,
            .initialWorkAreaGlobal = initialWorkArea,
        });

        if (debugLogsEnabled()) {
            std::ostringstream out;
            out << "[hymission] freeze niri overview viewport monitor=" << monitor->m_name
                << " viewport=" << rectToString(viewport)
                << " workArea=" << rectToString(initialWorkArea);
            debugLog(out.str());
        }
    }
}

void OverviewController::clearNiriOverviewViewports() {
    m_niriOverviewViewportSnapshots.clear();
}

Rect OverviewController::niriOverviewViewportForWorkspace(const PHLWORKSPACE& workspace) const {
    if (!workspace || !workspace->m_space)
        return {};
    ...
    const auto snapshot = std::find_if(m_niriOverviewViewportSnapshots.begin(), m_niriOverviewViewportSnapshots.end(),
                                       [&](const NiriOverviewViewportSnapshot& candidate) { return candidate.monitorId == monitor->m_id; });
    if (snapshot == m_niriOverviewViewportSnapshots.end()) {
        const CBox monitorBox = monitor->logicalBox();
        return makeRect(monitorBox.x, monitorBox.y, monitorBox.width, monitorBox.height);
    }

    if (!snapshot->changeLogged && !rectApproxEqual(snapshot->initialWorkAreaGlobal, liveWorkArea, 0.01)) {
        snapshot->changeLogged = true;
        if (debugLogsEnabled()) {
            std::ostringstream out;
            out << "[hymission] preserve niri overview viewport monitor=" << monitor->m_name
                << " viewport=" << rectToString(snapshot->viewportGlobal)
                << " initialWorkArea=" << rectToString(snapshot->initialWorkAreaGlobal)
                << " liveWorkArea=" << rectToString(liveWorkArea);
            debugLog(out.str());
        }
    }

    return snapshot->viewportGlobal;
}
...
void OverviewController::refreshNiriScrollingOverviewAfterLayoutScroll(const char* source, const PreviewRectSnapshot* previousPreviewRects) {
    ...
    // focus release, but during rapid movecol spam the native edge flag may not
    // be observable yet when the overview rebuilds.  In that case the normal
    ...
    if (afterLastEdgeReleaseRetarget && !afterLastNativeEdgeRetarget && previousPreviewRects && usesDirectNiriScrollingOverview(m_state) && scrolling && scrolling->m_scrollingData &&
        ...
        std::vector<PreviousColumnOverviewBounds> columnBounds;
        columnBounds.resize(scrolling->m_scrollingData->columns.size());

        const auto expandBounds = [](PreviousColumnOverviewBounds& entry, const Rect& rect) {
            if (!entry.hasBounds) {
                entry.bounds = rect;
            ...
            const double minX = std::min(entry.bounds.x, rect.x);
            const double minY = std::min(entry.bounds.y, rect.y);
            const double maxX = std::max(entry.bounds.x + entry.bounds.width, rect.x + rect.width);
            const double maxY = std::max(entry.bounds.y + entry.bounds.height, rect.y + rect.height);
            entry.bounds = makeRect(minX, minY, maxX - minX, maxY - minY);
        };
        ...
        for (std::size_t columnIndex = 0; columnIndex < scrolling->m_scrollingData->columns.size(); ++columnIndex) {
            ...
            for (const auto& targetData : column->targetDatas) {
                if (!targetData || !targetData->target || !scrollingTargetDataBelongsToWorkspace(targetData, workspace))
                    continue;
                ...
                const auto managedIt = std::find_if(m_state.windows.begin(), m_state.windows.end(), [&](const ManagedWindow& candidate) {
                    return candidate.window == window;
                });
                if (managedIt == m_state.windows.end())
                    continue;

                expandBounds(columnBounds[columnIndex], managedIt->targetGlobal);
            }
        }

        std::vector<std::size_t> validColumns;
        validColumns.reserve(columnBounds.size());
        for (std::size_t index = 0; index < columnBounds.size(); ++index) {
            if (columnBounds[index].hasBounds)
                validColumns.push_back(index);
        }

        if (validColumns.size() >= 2) {
            const std::size_t edgeRank = edgeRetargetPrevious ? 0 : validColumns.size() - 1;
            const std::size_t neighborRank = edgeRetargetPrevious ? 1 : validColumns.size() - 2;
            const Rect& edgeBounds = columnBounds[validColumns[edgeRank]].bounds;
            const Rect& neighborBounds = columnBounds[validColumns[neighborRank]].bounds;
            const double edgeCenter = horizontal ? edgeBounds.centerX() : edgeBounds.centerY();
            const double neighborCenter = horizontal ? neighborBounds.centerX() : neighborBounds.centerY();
            const double step = std::abs(edgeCenter - neighborCenter);
            if (std::isfinite(step) && step > 0.5) {
                const double signedStep = edgeRetargetNext ? -step : step;
                edgeCameraViewportDelta = horizontal ? Vector2D{signedStep, 0.0} : Vector2D{0.0, signedStep};
            }
        }

        if (debugLogsEnabled()) {
            std::ostringstream out;
            out << "[hymission] niri edge manual column delta resolve"
                << " source=" << (source ? source : "?")
                << " workspace=" << debugWorkspaceLabel(workspace)
                << " direction=" << (edgeRetargetPrevious ? "prev" : "next")
                << " columns=" << scrolling->m_scrollingData->columns.size()
                << " validColumns=" << validColumns.size()
    ...
    for (auto& managed : m_state.windows) {
        ...
        if (edgeViewportCameraPanWindow) {
            ...
            if (debugLogsEnabled() && edgeCameraViewportWindowLogs < 8) {
                ...
                std::ostringstream out;
                out << "[hymission] niri edge manual-retarget window target"
                    << " source=" << (source ? source : "?")
                    << " window=" << debugWindowLabel(managed.window)
    ...
    if (debugLogsEnabled()) {
        ...
        out << "[hymission] niri scrolling overview refresh source=" << (source ? source : "?") << " updated=" << updated
            ...
            << " edgeManualDelta=" << (edgeCameraViewportDelta ? 1 : 0)
            << " edgeManualDeltaUsed=" << (edgeCameraViewportDeltaUsed ? 1 : 0)
...
SDispatchResult OverviewController::runOverviewEditingDispatcher(const char* dispatcherName, DispatcherHandler* original, std::string args) {
    ...
    if (runDirectNiriDispatcherPath) {
        ...
        const bool forcedLeafEdgeFocusRelease = (isMoveColumnLayoutMessage || isDirectMoveColumnDispatcher) && leafMoveColumnTowardEdge;
        const bool nativeEdgeCameraFocusReleased = ((isMoveColumnLayoutMessage || isDirectMoveColumnDispatcher) && multiColumnEdgeCameraAfter &&
            (enteredNativeEdgeCameraAfterDispatch || movingIntoOrStayingAtEdge || (!preferred && !edgeMoveColumnAwayFromEdge))) || forcedLeafEdgeFocusRelease;
        if (debugLogsEnabled() && (isMoveColumnLayoutMessage || isDirectMoveColumnDispatcher) && multiColumnEdgeCameraAfter) {
            ...
            out << "[hymission] niri movecol edge camera after dispatch"
                ...
                << " leafTowardEdge=" << (leafMoveColumnTowardEdge ? 1 : 0)
                << " handOffInterruptedLeaf=" << (handOffInterruptedLeafToNativeEdge ? 1 : 0)
                << " forcedLeafEdgeRelease=" << (forcedLeafEdgeFocusRelease ? 1 : 0)
        ...
        if (insideRenderLifecycle()) {
            ...
            const char* relayoutSource = nativeEdgeCameraFocusReleased ?
...
Rect OverviewController::currentPreviewRect(const ManagedWindow& window) const {
    ...
    const auto dynamicNiriTiledResizeRect = [&]() -> std::optional<Rect> {
        ...
        const Rect viewportBase = niriOverviewViewportForWorkspace(workspace);
        const auto rowGeometry = scrollingOverviewTapeRowGeometryForWindow(window.window, sourceGlobal, anchorWindow, viewportBase);
        if (!rowGeometry)
            return std::nullopt;
        ...
        const auto anchorRowGeometry = scrollingOverviewTapeRowGeometryForWindow(anchorManaged->window, anchorSourceBase, anchorManaged->window, viewportBase);
        ...
        const auto backingViewport = std::find_if(m_state.emptyWorkspacePlaceholders.begin(), m_state.emptyWorkspacePlaceholders.end(),
                                                  [&](const EmptyWorkspacePlaceholder& candidate) {
                                                      return candidate.backingOnly && candidate.monitor == window.targetMonitor &&
                                                          candidate.workspaceId == workspace->m_id;
                                                  });
        const bool hasBackingViewport = backingViewport != m_state.emptyWorkspacePlaceholders.end() &&
            backingViewport->targetGlobal.width > 1.0 && backingViewport->targetGlobal.height > 1.0;
        const double viewportX = hasBackingViewport ? backingViewport->targetGlobal.x :
            anchorPreviewBase.centerX() - (anchorSourceGlobal.centerX() - rowGeometry->baseGlobal.x) * scale;
        const double viewportY = hasBackingViewport ? backingViewport->targetGlobal.y :
            anchorPreviewBase.centerY() - (anchorSourceGlobal.centerY() - rowGeometry->baseGlobal.y) * scale;
...
Rect OverviewController::niriWorkspaceSurfaceRect(const State& state, const EmptyWorkspacePlaceholder& background, const Rect& viewportRect,
    ...
    const Rect workspaceRect = niriOverviewViewportForWorkspace(workspace);
...
OverviewController::State OverviewController::buildState(const PHLMONITOR& monitor, ScopeOverride requestedScope, const std::vector<WorkspaceOverride>& workspaceOverrides,
    ...
    if (allowDirectNiriOverviewLayout) {
        ...
        for (const auto& candidateMonitor : state.participatingMonitors) {
            ...
            if (baseWorkspace && baseWorkspace->m_space) {
                const Rect baseGlobal = niriOverviewViewportForWorkspace(baseWorkspace);
        ...
        for (const auto& candidate : candidates) {
            ...
            if (candidateIsTiledScrolling) {
                ...
                if (const auto rowGeometry = scrollingOverviewTapeRowGeometryForWindow(
                        candidate, candidateAnchorGlobal, {}, niriOverviewViewportForWorkspace(candidate->m_workspace)))
    ...
    const auto niriOverviewSlotForSource = [&](const PHLWINDOW& window, const PHLMONITOR& targetMonitor, const Rect& sourceGlobal, const Rect& baseGlobal,
            ...
            if (!anchorOverride && layoutWorkspace) {
                ...
                if (anchorWindow && anchorWindow->m_workspace == layoutWorkspace) {
                    ...
                    if (const auto anchorRowGeometry = scrollingOverviewTapeRowGeometryForWindow(
                            anchorWindow, anchorSourceGlobal, anchorWindow, niriOverviewViewportForWorkspace(layoutWorkspace)))
        ...
        // Pinned/floating overlays are not scrolling-tape members. They must not
        // claim the per-workspace wallpaper/backing placeholder, because their
        // slot path does not carry the scrolling tape overflow axis. If an
        // overlay is processed before a real tiled scrolling window, claiming
        // this here makes the wallpaper use the overlay scale and appear smaller
        // than the rest of the workspace viewports. Let tiled scrolling windows
        // create the backing placeholder; if the workspace has only overlays,
        // the empty-workspace placeholder pass below creates a stable
        // workspace-sized viewport.
    ...
    const auto niriFloatingOverviewSlotForWindow = [&](const PHLWINDOW& window, const PHLMONITOR& targetMonitor, const Rect& sourceGlobal,
        ...
        Rect baseGlobal = niriOverviewViewportForWorkspace(layoutWorkspace);
        if (baseGlobal.width <= 1.0 || baseGlobal.height <= 1.0)
    ...
    const auto niriScrollingOverviewSlotForWindow = [&](const PHLWINDOW& window, const PHLMONITOR& targetMonitor, const Rect& sourceGlobal,
        ...
        if (const auto rowGeometry = scrollingOverviewTapeRowGeometryForWindow(
                window, sourceGlobal, layoutAnchorWindow, niriOverviewViewportForWorkspace(window->m_workspace))) {
            ...
            baseGlobal = niriOverviewViewportForWorkspace(window ? window->m_workspace : PHLWORKSPACE{});
        }
    ...
    if (allowDirectNiriOverviewLayout) {
        ...
        for (const auto& workspace : state.managedWorkspaces) {
            ...
            const bool hasManagedWindow = std::ranges::any_of(state.windows, [&](const ManagedWindow& managed) {
                ...
                // Pinned/floating overlays are not occupants of the focused
                // scrolling workspace lane. Counting them here suppresses the
                // stable empty/backing viewport for overlay-only workspaces,
                // leaving the overlay card's smaller viewport as the only
                // wallpaper rect.
                if (managed.window->m_pinned || managed.isPinned || managed.isNiriFloatingOverlay || isFloatingOverviewWindow(managed.window))
                    return false;
